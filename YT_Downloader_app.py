...
def download_video(url, format_choice):
    ...
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            ext = "mp4" if format_choice == "mp4" else "mp3"
            filename = f"{title}.{ext}"

            # Actually download into memory buffer
            ydl.download([url])
...
if st.button("Download"):
    if video_url:
        with st.spinner("Downloading..."):
            filename, download_url, error = download_video(video_url, format_choice)
            if error:
